...
namespace all_tests
{
	...
	public:
		...
		TEST_METHOD(test_02b)
		{
			...
			std::vector<int> v(10);
			int value = -1;
			std::generate(v.begin(), v.end(), [&value]() {return value += 2; });
		...
		TEST_METHOD(test_07b)
		{
			...
			std::replace_if(s.begin(), s.end(), [](char& l) {
				return "aeiou"s.find(l) != std::string::npos; }, 'x');
		...
		TEST_METHOD(test_12)
		{
			...
			auto smallest_difference = *std::min_element(differences.begin() + 1, differences.end());
